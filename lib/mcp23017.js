...
var MCP23017 = (function() {
  ...
  async function MCP23017 (config) {
    ...
    this.wire = config.wire || i2c.openSync(this.device);
  ...
  MCP23017.prototype.reset = async function () {
    ...
    for (var i = 0; i < 2; i++) {
      this.inten[i] = 0x00;
      await this._send(INTEN_ADDR[i], [this.inten[i]]);
      this.defval[i] = 0x00;
      await this._send(DEFVAL_ADDR[i], [this.defval[i]]);
      this.intcon[i] = 0x00;
      await this._send(INTCON_ADDR[i], [this.intcon[i]]);
    }
    await this._send(IOCON_ADDR, [0x00])
    this._initGpioA();
    this._initGpioB();
  };
  ...
  */
  MCP23017.prototype.interruptMode = async function (mirror, mode) {
    if (mode !== this.ACTIVE_LOW && mode !== this.ACTIVE_HIGH && mode !== this.OPEN_DRAIN) {
      console.error('invalid value', mode);
      return;
    }
    if (mirror == true) {
      await this._send(IOCON_ADDR, [0x40 | mode]);
    } else {
      await this._send(IOCON_ADDR, [mode]);
    }
  };
  ...
  /*
    configures an pin interrupt
  */
  MCP23017.prototype.attachInterrupt = async function (pin, mode) {
    ...
    await this._setInterrupt(pin >= 8 ? pin - 8 : pin, mode, pin >= 8 ? 1 : 0);
  };
  
  MCP23017.prototype._setInterrupt = async function (pin, mode, port) {
    ...
    if (mode === this.CHANGE) {
      if ((this.intcon[port] & pinHexMask) === pinHexMask) {
        ...
        await this._send(INTCON_ADDR[port], [this.intcon[port]]);
      } else {
      ...
      if (mode === this.HIGH) {
        if ((this.defval[port] & pinHexMask) === pinHexMask) {
          ...
          await this._send(DEFVAL_ADDR[port], [this.defval[port]]);
        } else {
    ...
    if ((this.inten[port] & pinHexMask) !== pinHexMask) {
      ...
      await this._send(INTEN_ADDR[port], [this.inten[port]]);
    } else {
      this.log('pin \'' + pin + '\' interrupt already enabled');
    }
  }
  
  MCP23017.prototype.disableInterrupt = async function (pin) {
    if (isNaN(pin)) {
      console.error('pin is not a number:', pin);
      return;
    } else if (pin > 15 || pin < 0) {
  ...
  */
  MCP23017.prototype.getInterruptCause = async function (port, callback) {
    let _port = port // make const copy...

    //read one byte from the right register (A or B)
    await this._read(INTF_ADDR[port], 1, function (err, registerValue) {
      if (err) {
        console.error(err);
  ...
  */
  MCP23017.prototype.getInterruptCapture = async function (port, callback) {
    let _port = port // make const copy...

    //read one byte from the right register (A or B)
    await this._read(INTCAP_ADDR[port], 1, function (err, registerValue) {
      if (err) {
        console.error(err);
        callback(_port, err, null);
      } else {
  ...
  */
  MCP23017.prototype.pinMode = async function (pin, dir) {
    ...
    //delegate to function that handles low level stuff
    await this._setGpioDir(pin >= 8 ? pin - 8 : pin, dir, pin >= 8 ? REGISTER_GPIOB : REGISTER_GPIOA, pin >= 8 ? REGISTER_GPIOB_PULLUP : REGISTER_GPIOA_PULLUP);
  };
  ...
  */
  ...
  MCP23017.prototype._setGpioAPinValue = async function (pin, value) {
    var pinHexMask = Math.pow(2, pin);
    if (value === 0) {
      if ((this.gpioAState & pinHexMask) === pinHexMask) {
        this.gpioAState = this.gpioAState ^ pinHexMask;
        await this._send(WRITE_GPIOA_ADDR, [this.gpioAState]);
      }
    }
    if (value === 1) {
      if ((this.gpioAState & pinHexMask) !== pinHexMask) {
  ...
  MCP23017.prototype._setGpioBPinValue = async function (pin, value) {
    var pinHexMask = Math.pow(2, pin);
    if (value === 0) {
      if ((this.gpioBState & pinHexMask) === pinHexMask) {
        this.gpioBState = this.gpioBState ^ pinHexMask;
        await this._send(WRITE_GPIOB_ADDR, [this.gpioBState]);
      }
    }
    if (value === 1) {
      if ((this.gpioBState & pinHexMask) !== pinHexMask) {
  ...
  MCP23017.prototype.digitalWrite = async function (pin, value) {
    ...
    if (isNaN(pin)) {
      ...
      //Port A
      await this._setGpioAPinValue(pin, value);
    } else {
      //Port B
      pin -= 8;
      await this._setGpioBPinValue(pin, value);
    }
  };
  
  MCP23017.prototype.writePort = async function (port, value) {
    ...
    if (isNaN(port)) {
      ...
      await this._send(WRITE_GPIOB_ADDR, [value]);
    } else {
      //Port A
      this.gpioAState = value
      await this._send(WRITE_GPIOA_ADDR, [value]);
    }
  };

  MCP23017.prototype.digitalRead = async function (pin, callback) {
    ...
    //read one byte from the right register (A or B)
    await this._read(register, 1, function (err, registerValue) {
      if (err) {
        console.error(err);
  ...
  MCP23017.prototype.readPort = async function (port, callback) {
    let _port = port // make const copy...
  ...
  MCP23017.prototype._initGpioA = async function () {
    await this._send(REGISTER_GPIOA, [this.dirAState]); //Set Direction to Output
    await this._send(WRITE_GPIOA_ADDR, [this.gpioAState]); //clear all output states
  };

  MCP23017.prototype._initGpioB = async function () {
    await this._send(REGISTER_GPIOB, [this.dirBState]); //Set Direction to Output
    await this._send(WRITE_GPIOB_ADDR, [this.gpioBState]); //clear all output states
  };

  MCP23017.prototype._send = async function (cmd, values) {
    var buff = Buffer.from(values);
    await this.wire.writeI2cBlock(this.address, cmd, buff.length, buff);
  };

  MCP23017.prototype._read = async function (cmd, length, callback) {
    var buff = Buffer.alloc(length);
    await this.wire.readI2cBlock(this.address,cmd,length,buff,function (err, bytesRead, buffer) {
        if (err) {
          console.error(err);
          callback(err, null);
        } else {
